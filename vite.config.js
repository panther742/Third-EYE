 ...
 */
/**
 * Keyless geocoder proxy for the LOCATION search box.
 * Forwards /api/geocode?q=… to Nominatim server-side so typed place searches
 * resolve on static/sandboxed/embedded hosts where the browser itself cannot
 * reach nominatim.openstreetmap.org. Returns compact JSON rows:
 *   [{lat, lon, label, bbox:[south,north,west,east]}]
 * Mirrors the searchAndFlyToKeyless contract in src/locations.js.
 */
    ...
    configureServer(server) {
      server.middlewares.use('/api/geocode', async (req, res) => {
        ...
        try {
          ...
          if (!q || q.length > 120) return send(400, { results: [] });
          const upstream = await fetch(
            'https://nominatim.openstreetmap.org/search?format=jsonv2&limit=6&addressdetails=0&q='
            + encodeURIComponent(q),
            { headers: { 'User-Agent': 'third-eye-globe/1.0', Accept: 'application/json' }, signal: AbortSignal.timeout(9000) },
          );
          if (!upstream.ok) return send(502, { results: [], error: `nominatim ${upstream.status}` });
          ...
          send(502, { results: [], error: String(error?.message || error).slice(0, 80) });
        }
      });
    },
    configurePreviewServer(server) {
...
export default defineConfig(({ mode }) => {
  ...
  return {
    ...
    server: {
      ...
      // Sandboxed preview hosts (e.g. remote dev containers that frame the app)
      // can set GEV_ALLOW_EMBED=1 to relax frame protection. Default stays DENY.
      headers: (env.GEV_ALLOW_EMBED === '1')
