...
## [1.0.0] — THIRD EYE (India fork)

### Added — India custom build
- **THIRD EYE** branding end-to-end (title, HUD, loader, README)
- **Surat-first boot** — cinematic entry over Tapi riverfront; stale URL-hash restore removed so every reload is a clean home view
- **🌐 HOME toggle** — full-globe ⇄ Surat city; releases every camera owner (tracked flight/satellite/cockpit) before gliding home, so the camera never parks over farmland
- **Keyless world search** — Nominatim fallback when no Google key; any place name flies
- **10 Indian city packs** — Surat, Mumbai, Delhi, Ahmedabad, Bengaluru, Hyderabad, Chennai, Kolkata, Jaipur, Agra with landmark POIs (Taj Mahal, India Gate, Charminar, Statue of Unity…)
- **🇮🇳 INDIA CAMS board** — official public broadcasts (Dwarkadhish live embed, Somnath, Golden Temple, Surat Smart City)
- **Google-Maps-style hybrid view** — Esri reference overlays (place labels + roads) composited on satellite imagery; per-stack lifecycle
- **Scope mask default OFF** — full open view; re-enable via DISPLAY → Scope
- **Live data layers default ON** — flights, military, satellites, earthquakes, rocket launches (storage key bumped to v3 so old OFF states don't linger)
- **CCTV opens nearest camera globally** (haversine), with an honest coverage hint
- **Preview-embed flag** — `GEV_ALLOW_EMBED=1` relaxes frame-ancestors for sandboxed previews (default stays DENY)
