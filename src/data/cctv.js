...
const cctvLayer = {
  ...
  enable() {
    ...
    if (!_activeCameraId && _records.length) {
      // LIVE-DEFAULTS (local patch): open on the camera NEAREST the viewer
      // (haversine, global — from India that's London, not the first Austin
      // row) so the frame panel shows a live feed immediately no matter
      // where the globe is parked. Catalog cities: Austin, California
      // districts, London.
      _activeCameraId = nearestCameraIdToViewer() || _records[0].camera.id;
      _autoHopSuspended = false;
    }
    const activeRecord = getActiveRecord();
    if (activeRecord) {
      ...
      try {
        const meta = document.getElementById('cctv-meta');
        if (meta) {
          meta.textContent = `Live: ${activeRecord.camera.name} — ${activeRecord.camera.city}. Cameras exist only in Austin, California & London — use FOCUS to fly there.`;
        }
      } catch { /* meta hint is best-effort */ }
    }
