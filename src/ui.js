...
import { flyToAustin, flyToHomeCity } from './camera.js';
...
export class StyleManager {
   ...
   */
  resetToGlobeView() {
    // LIVE-DEFAULTS (local patch): the 🌐 icon is a HOME toggle. Already at
    // full-globe scale? Release EVERY camera owner first (tracked flight,
    // satellites, cockpit, orbit) — otherwise tracking re-grabs the camera
    // mid-flight and parks it over farmland — then glide home to Surat in one
    // smooth move. Below globe scale? Do the original zoom-out to the globe.
    try {
      const currentHeight = this.viewer.camera.positionCartographic?.height;
      if (Number.isFinite(currentHeight) && currentHeight > 12000000) {
        this._stampNavigation();
        interruptCameraMotion('reset-home');
        this._stopOrbit();
        this.cockpitView?.exit({ restoreTracking: false });
        try {
          militaryAwarenessLayer.releaseCameraOwnership?.({ origin: 'tool' });
        } catch {
          try { flightsLayer.stopTracking?.({ origin: 'tool' }); } catch { /* best-effort */ }
          try { militaryFlightsLayer.stopTracking?.({ origin: 'tool' }); } catch { /* best-effort */ }
          try { aisLiveVesselsLayer.clearSelection?.(); } catch { /* best-effort */ }
        }
        try { satellitesLayer.stopTracking?.({ origin: 'tool' }); } catch { /* best-effort */ }
        try { rocketLaunchesLayer.releaseCameraOwnership?.(); } catch { /* best-effort */ }
        this.viewer.trackedEntity = undefined;
        this.viewer.camera.cancelFlight();
        this.viewer.camera.lookAtTransform(Cesium.Matrix4.IDENTITY);
        flyToHomeCity(this.viewer);
        return Promise.resolve({
          ok: true,
          action: 'home_view',
          cancelled: false,
          heightKm: 2,
          centeredOn: { latitude: 21.1702, longitude: 72.8311 },
        });
      }
    } catch { /* fall through to the original globe reset */ }
