...
// LIVE-DEFAULTS (local patch): Google-Maps-style HYBRID view. The raw satellite
// basemap has no place names or roads, so zooming into a country lands on
// anonymous farmland. Esri's free reference services are transparent PNG tile
// overlays — city/country labels and road markings — drawn ON TOP of the
// imagery, exactly how Google's hybrid mode is built. Same provider already
// used for the basemap; same terms.
const ESRI_REFERENCE_PLACES_URL =
  'https://services.arcgisonline.com/ArcGIS/rest/services/Reference/World_Boundaries_and_Places/MapServer';
const ESRI_REFERENCE_ROADS_URL =
...
export class MapStackController {
  ...
  async _activateGlobeStack(stack, gen) {
    ...
    // LIVE-DEFAULTS (local patch): hybrid reference overlays for the Esri
    // satellite stack — place/country labels + roads on top of the imagery so
    // zooming into any country reads like a real map. OSM already carries its
    // own labels; photoreal/ion stacks manage their own styling.
    this._esriReferenceLayers = [];
    if (resolution.effectiveStackId === 'esri-imagery') {
      try {
        const placesProvider = new Cesium.UrlTemplateImageryProvider({
          url: `${ESRI_REFERENCE_PLACES_URL}/tile/{z}/{y}/{x}`,
          credit: 'Powered by Esri',
          enablePickFeatures: false,
        });
        const roadsProvider = new Cesium.UrlTemplateImageryProvider({
          url: `${ESRI_REFERENCE_ROADS_URL}/tile/{z}/{y}/{x}`,
          credit: 'Powered by Esri',
          enablePickFeatures: false,
        });
        const placesLayer = this.viewer.imageryLayers.addImageryProvider(placesProvider);
        const roadsLayer = this.viewer.imageryLayers.addImageryProvider(roadsProvider);
        placesLayer.alpha = 0.95;
        roadsLayer.alpha = 0.85;
        this._esriReferenceLayers.push(placesLayer, roadsLayer);
      } catch (error) {
        console.warn('[MapStack] Reference overlays unavailable:', error?.message || error);
        this._esriReferenceLayers = [];
      }
    }
  ...
  _removeImageryLayer() {
    ...
    // LIVE-DEFAULTS (local patch): drop the hybrid reference overlays with the
    // base layer so a stack switch never leaves orphaned label layers behind.
    if (Array.isArray(this._esriReferenceLayers) && this._esriReferenceLayers.length) {
      for (const overlay of this._esriReferenceLayers) {
        try { this.viewer.imageryLayers.remove(overlay, false); } catch { /* best-effort */ }
      }
      this._esriReferenceLayers = [];
    }
