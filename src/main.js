...
import { initIndiaCams } from './indiaCams.js';
...
async function init() {
  ...
  try {
    ...
    // If no share link state, do default fly-to the home city (Surat, India)
    if (!styleManager.hasShareState) {
      loaderStatus.textContent = 'Flying to Surat, India...';
      flyToAustin(viewer);
    } else {
    ...
    // LIVE-DEFAULTS (local patch): India live-cams board (official public broadcasts)
    initIndiaCams(viewer);
    ...
    console.error("Third Eye initialization failed:", error);
