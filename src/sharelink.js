...
export class ShareLinkManager {
  constructor(viewer, {
    ...
    // LIVE-DEFAULTS (local patch): full open view by default ("jaise aankh se
    // dekhte hain") — the circular keyhole mask made the map feel like a game.
    // Re-enable any time: DISPLAY panel → Scope.
  ...
  parseInitialHash() {
    ...
    // LIVE-DEFAULTS (local patch): the app no longer writes the camera into
    // the address bar, but a stale auto-written hash from an older session
    // may still sit in the URL. Genuine shared links carry the copy-time `at`
    // timestamp; auto-written ones never did. Ignore the latter so every
    // reload gets the fresh Surat home boot.
    if (!params.has(SHARE_CREATED_AT_PARAM)) return null;
  ...
  _updateHash() {
    if (this._destroyed || this._initialRestorePending) return;
    // LIVE-DEFAULTS (local patch): keep the address bar clean. Writing the
    // camera into the URL made every reload restore the previous position,
    // which overrode the Surat home boot. Share links still work: copyLink
    // builds the hash in memory, and parseInitialHash still restores any
    // hash pasted into the URL.
    const params = this._buildHashParams();
    if (!params) return;
    this._lastBuiltParams = params.toString();
  }
